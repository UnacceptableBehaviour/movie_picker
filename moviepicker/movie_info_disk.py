...
class MMdia:              # (object) not needed in 3.x
  ...
  def __init__( self, file_path=None ):
    ...
    # should check here to see it already present! TODO
    if file_path != None:
  ...
  @staticmethod
  def refresh_media_files_information(root_dir, reset_library=False):
    
    limit_to = 2000 #2000
    count = 0
    
    # hard reset
    if reset_library == True:
...
# helpers
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
#mmdia_root = Path('/Volumes/FAITHFUL500')            # local mount FAITHFUL500
#mmdia_root = Path('/Volumes/Home Directory/MMdia/')  # remote mount point - dev FAITHFUL500
mmdia_root = Path('/Volumes/time_box_2018/movies/')
...
# TODO
# TODO
# separate out from class - creating technical debt here!
# quick hack
def get_MMdia_lib(pickle_path=PICKLED_MEDIA_LIB_FILE):
  media_lib = None
  lib = MMdia()
...
# class MMdia_lib():
# 
#   def __init__( self, file_path=PICKLED_MEDIA_LIB_FILE ):
#     if file_path.exists():
#       print(f"\n\nUN-PICKLING from {PICKLED_MEDIA_LIB_FILE}")
#       with open(file_path, 'rb') as f:
#         # MMdia = pickle.load(f)
#         MMdia.load_media_files_information(pickle.load(f))
#         self.media_lib = MMdia.media_files    
# 
#   def __str__(self):    
#     return 'MMdia_lib::def __str__'
# 
#   def __repr__(self):   
#     return 'MMdia_lib::def __repr__'
